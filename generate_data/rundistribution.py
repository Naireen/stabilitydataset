...
def run_distribution(sim_id, integrator="whfast", dt=None, maxorbs=1.e3, betamin=1., betamax=30., runstr=None, kickseed=None):
    if dt is None:
        dt = 2.*math.sqrt(3)/100. # dt approx 3.5 % of innermost orbital period

    datapath = '../data/distributions/'
    ...
    if runstr:
        sim.save(datapath+'initial_conditions/ic'+runstr)
        sim.initSimulationArchive(datapath+'simulation_archives/sa'+runstr, interval=maxorbs/1000.)
    ...
    if runstr:
        sim.save(datapath+'final_conditions/fc'+runstr)

    return (sim.t, Eerr, time.time()-t0)

sim_id = int(sys.argv[1])
runstring = sys.argv[2]
kickseed = int(sys.argv[3])
run_distribution(sim_id, runstr = runstring, kickseed = kickseed)
